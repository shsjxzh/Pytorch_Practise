    ...
    for epoch in range(num_epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                A_output.detach_()
                input_vector = torch.cat((IC_id, A_output), 1)
            ...
            fake_images = G(input_vector)
            # IC_fake = IC_gen(fake_images)
            IC_fake = IC(fake_images)
            _, fake_feature = D(fake_images)
            _, attribute_feature = D(attribute_images)

            LGR_loss = 0.5 * ((fake_images - identity_images)**2).sum()
            LGC_loss = 0.5 * ((IC_fake - IC_id)**2).sum()
            ...
            if (iter_count % SHOW_STEP == 0):
                ...
                print('LKL: {:.4}, LGR:{:.4}, LGC, :{:.4}, LGC, :{:.4}'.format(LKL_loss.data.item(), LGR_loss.data.item(), LGC_loss.data.item(), LGD_loss.data.item()) )
